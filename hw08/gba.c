...
void drawFullScreenImageDMA(const u16 *image) {
  ...
  DMA[3].cnt = DMA_ON | WIDTH * HEIGHT | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;
}
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  ...
  for (int count = 0; count < height; count++) {
        DMA[3].cnt = 0;
        DMA[3].src = &image[OFFSET(count, 0, width)];
        DMA[3].dst = &videoBuffer[OFFSET(row + count, col, 240)];
...
void fillScreenDMA(volatile u16 color) {
  ...
  DMA[3].src = &color;
  DMA[3].dst = videoBuffer;
  DMA[3].cnt = DMA_ON | HEIGHT * WIDTH | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT;
}
